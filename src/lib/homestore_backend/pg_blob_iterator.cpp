...
void HSHomeObject::PGBlobIterator::create_pg_shard_snapshot_data(sisl::io_blob_safe& meta_blob) {
    ...
    std::vector< ::flatbuffers::Offset< Member > > member_entries;
    member_entries.reserve(pg_info.members.size());
    for (auto& member : pg_info.members) {
        std::vector< std::uint8_t > uuid(member.id.size());
        std::copy(member.id.begin(), member.id.end(), uuid.begin());
        member_entries.push_back(CreateMember(
            builder, builder.CreateVector(uuid),
            builder.CreateVector(r_cast< uint8_t* >(const_cast< char* >(member.name.data())), member.name.size())));
    }
    ...
    auto pg_entry = CreatePGInfoEntry(builder, pg_info.id, 0 /* priority*/, builder.CreateVector(uuid),
                                      builder.CreateVector(member_entries));
...
int64_t HSHomeObject::PGBlobIterator::get_next_blobs(uint64_t max_num_blobs_in_batch, uint64_t max_batch_size_bytes,
    ...
    while (true) {
        ...
        if (!r) { return -1; }
        auto& blob_info_vec = r.value();
        for (auto& info : blob_info_vec) {
        ...
        if (blob_info_vec.empty()) {
            // We read all the blobs in the current shard
            end_of_shard = true;
            cur_shard_seq_num_++;
            cur_blob_id_ = 0;
            break;
