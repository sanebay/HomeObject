...
std::shared_ptr< homestore::snapshot_context > ReplicationStateMachine::last_snapshot() { return m_snapshot_context; }
...
void ReplicationStateMachine::write_snapshot_data(std::shared_ptr< homestore::snapshot_context > context,
                                                  std::shared_ptr< homestore::snapshot_data > snp_data) {
    if (context) {
        LOGI("write_snapshot_data context not null");
    } else {
        LOGI("write_snapshot_data is null");
    }

    if (snp_data) {
        LOGI("write_snapshot_data snp_data not null");
    } else {
        LOGI("write_snapshot_data is null");
    }
    ...
    if (obj_id == 0) {
        ...
        LOGI("write_snapshot_data 3");
        home_object_->create_pg(std::move(pg_info)).get();
        LOGD("Write snapshot create pg {} result {}", snp->pg()->pg_id(), log_str);
        // RELEASE_ASSERT(!!r, "create pg failed");
        return;
    }

    LOGD("Writing blob values size {}", snp_data->blob.size());
    ...
    for (auto const& b : *(snp->data_array())) {
        ...
        auto r = home_object_->local_put(b->shard_id(), std::move(blob)).get();
        if (!r) {
            LOGD("Write snapshot put blob shard {} failed result {} {}", b->shard_id(), r.error(), log_str);
        } else {
            LOGD("Write snapshot put blob shard {} result {} {}", b->shard_id(), r.value(), log_str);
        }
    }
