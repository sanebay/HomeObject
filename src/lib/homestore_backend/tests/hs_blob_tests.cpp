...
int read_snapshot_data(homeobject::HSHomeObject* home_object, peer_id_t replica_set_uuid,
...
void write_snapshot_data(homeobject::HSHomeObject* home_object, std::shared_ptr< homestore::snapshot_context > context,
                         std::shared_ptr< homestore::snapshot_data > snp_data) {
    ...
    for (auto const& b : *(snp->data_array())) {
        ...
        RELEASE_ASSERT(!!r, "put blob failed");
    }
...
TEST_F(HomeObjectFixture, PGBlobIterator) {
    ...
    while (!snapshot_data->is_last_obj) {
        read_snapshot_data(ho, pg1->pg_info_.replica_set_uuid, context, snapshot_data);
        write_snapshot_data(ho, context, snapshot_data);
    }
